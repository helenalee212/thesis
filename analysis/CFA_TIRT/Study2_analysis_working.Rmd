...
fc2_blocks <- 
  ...
  set_block(items=c("GF52", "GF53", "GF54"), traits = c("AN", "CO", "DE")) +
  set_block(items=c("GF55", "GF56", "GF57"), traits = c("NE", "PS", "DI")) +
  set_block(items=c("GF58", "GF59", "GF60"), traits = c("AN", "PS", "DE"))
...
== weighted least square mean and variance adjusted (WLSMV) estimators in R
...
In orginial model (Geonole, 2016), initial ideniticaiton also failed. 
They contrained correlation between AN (Antagonism), DI (Disinhibition), and NE (Negative Affect).
...
# fix cor of AN DI NE to the estimated values in SS model, AN-DI = 0.649; AN-NE = 0.236; DI-NE = 0.635
...
fc2_fit <- fit_TIRT_lavaan(fc2_triplets, estimator = "WLSMVS")
...
# View results
print(fc2_fit)
predict(fc2_fit, newdata = fc2_triplets)

# store results 
fc2_traits_TIRT <- predict(fc2_fit, newdata = fc2_triplets)
...
fc1_fit <- fit_TIRT_lavaan(fc1_triplets, estimator = "WLSMVS")
```
...
# store results 
fc1_traits_TIRT <- predict(fc1_fit, newdata = fc1_triplets)

```

```{r  fit}
# View results
summary(fc1_fit, fit.measures = TRUE)
fc1_summary
